...
class ProjectUsersView(Resource):
    ...
    def post(self, project_id):
        ...
        user = User.find_first(
            email=validated_project_user_data.get('email', None))

        if not user:
            # register anonymous user
            anonymous_user_email = validated_project_user_data.get(
                'email', None)
            anonymous_user_exists = AnonymousUser.find_first(
            ...
            if not resend_invite:
                new_anonymous_user = AnonymousUser(
                    email=anonymous_user_email, project_id=project.id, role=role)
    ...
    def get(self, project_id):
        ...
        project_anonymous_user_data, errors = anonymous_user_schema.dumps(
            project_anonymous_users)
        if errors:
            return dict(status="fail", message="Internal Server Error"), 500
        ...
        return dict(
            ...
            data=dict(project_users=json.loads(project_user_data),
                      project_anonymous_users=json.loads(project_anonymous_user_data))
        ), 200
    ...
    def delete(self, project_id):
        ...
        if not user:
            ...
            deleted_anonymous_user = AnonymousUser.delete(
...
class ProjectUsersTransferView(Resource):
    ...
    def post(self, project_id):
        ...
        owner_user = ProjectUser.find_first(
            role='owner', project_id=project.id)
...
class ProjectUsersHandleInviteView(Resource):
    ...
    def patch(self, project_id):
        ...
        project_user_schema = ProjectUserSchema(
            partial=True, exclude=["email", "role"])
        ...
        invite_status = validated_project_user_data.get(
            'accepted_collaboration_invite', None)
        ...
        updated = ProjectUser.update(
            existing_user, accepted_collaboration_invite=invite_status)
...
class ProjectFollowingView(Resource):
    ...
    def post(self, project_id):
        ...
        existing_project_follow = ProjectFollowers.find_first(
        ...
        new_project_follow = ProjectFollowers(
            user_id=current_user_id, project_id=project_id)
