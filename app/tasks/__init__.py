...
celery_app = Celery(__name__, broker=redis_url,
                    backend=redis_url, include=['app.tasks', 'app.helpers.activity_logger'])
...
def setup_periodic_tasks(**kwargs):
    ...
    celery_app.add_periodic_task(
        crontab(minute="*"), check_app_states.s(), name='check app statuses')
...
def updateScheduler():
    ...
    try:
        sq = db.session.query(CreditAssignment.user_id, func.max(CreditAssignment.expiry_date).label(
            'expiry_date')).group_by(CreditAssignment.user_id).subquery()
        ...
        credit_assignments_json, errors = credit_assignment_schema.dumps(
        ...
        if len(arr) > 0:
            for i in arr:
                # update query
                sql = db.session.query(Credit).filter(Credit.user_id == i['user_id'], Credit.amount == i['amount']).update(
                    {Credit.amount: 0}, synchronize_session=False)
                db.session.commit()
